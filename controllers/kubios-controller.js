...
const syncTimeVaryingData = async (req, res, next) => {
    try {
      ...
      console.log('detailData.measure:', JSON.stringify(detailData.measure, null, 2));
      console.log('channels:', JSON.stringify(detailData.measure?.channels, null, 2));
      console.log('Kaikki channels:', detailData.measure.channels.map(c => ({ type: c.type, has_data_url: !!c.data_url })));
...
const getTimevaryingData = async (req, res, next) => {
  try {
    ...
    const row = rows[0];
    const tv = typeof row.timevarying_data === 'string'
 ...
 export {getUserData, getUserInfo, syncTimeVaryingData, getTimevaryingData};